...
var getUserByEnentKey = (eventKey) => {
	var result = db.find("SELECT headimgurl, nick , (select create_time from T_WECHAT_ORDER where openid = U.openid ORDER BY CREATE_TIME LIMIT 1 )as create_time FROM T_WECHAT_USER as U WHERE  U.flag != '1' AND U.order_count > 0 AND U.eventKey = 'qrscene_"+ eventKey +"' or U.eventKey = '"+ eventKey+"' AND U.order_count > 0  ORDER BY U.order_count DESC");
	return result;
}
