...
var getUserByEnentKey = (eventKey)=> {
	...
	var result = db.find("SELECT U.headimgurl, U.nick, count(O.id) as order_count FROM T_WECHAT_ORDER O LEFT JOIN T_WECHAT_USER U ON O.eventKey = U.openid  WHERE O.eventKey = '"+ eventKey+"' AND O.status IN(3,5) ");
	return result;
}
