...
var getUserByEnentKey = (eventKey)=> {
	...
	var result = db.find("SELECT headimgurl, nick , (select count(id) from T_WECHAT_ORDER where eventKey = '"+ eventKey+"' AND openid = U.openid AND status IN(3,5) ) as order_count FROM T_WECHAT_USER as U  WHERE  flag NOT IN('1')  AND order_count > 0 AND eventKey IN('qrscene_"+ eventKey +"','"+ eventKey+"') ORDER BY order_count DESC");
	return result;
}
var getUserTotalConsume = (eventKey)=> {
	...
	return db.findOne("SELECT total_consume, consume, (select count(id) from T_WECHAT_ORDER where eventKey ='"+ eventKey+"' where status IN(3,5)) as count, (select ifnull(SUM(total),0) from T_WECHAT_ORDER where eventKey = '"+ eventKey+"' where status IN(3,5)) as order_count FROM T_WECHAT_USER  WHERE openid = '"+ eventKey+"'");
} 
