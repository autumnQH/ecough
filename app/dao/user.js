...
var getUserTotalConsume = (eventKey)=> {
	...
	return db.findOne("SELECT total_consume, consume, (select count(id) from T_WECHAT_ORDER where eventKey ='"+ eventKey+"' AND status IN(3,5)) as count, (select ifnull(SUM(total),0) from T_WECHAT_ORDER where eventKey = '"+ eventKey+"' AND status IN(3,5)) as order_count FROM T_WECHAT_USER  WHERE openid = '"+ eventKey+"'");
} 
