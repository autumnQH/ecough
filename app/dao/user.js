...
var getUserByEnentKey = (eventKey) => {
	var result = db.find("SELECT headimgurl, nick , (select create_time from T_WECHAT_ORDER where openid = U.openid ORDER BY CREATE_TIME LIMIT 1 )as create_time FROM T_WECHAT_USER as U WHERE  flag NOT IN('1')  AND order_count > 0 AND eventKey IN('qrscene_"+ eventKey +"','"+ eventKey+"') ORDER BY order_count DESC");
	return result;
}
