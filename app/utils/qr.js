...
exports.qr = async function(userinfo) {
  ...
  .pipe(fs.createWriteStream(__dirname+'/'+userinfo.openid+'logo.jpeg').on('close', function(err, data) {
...
exports.qr_logo = async function(userinfo, qrurl){
  return new Promise(function(resolve, reject) {
    ...
    var logo = images(__dirname+'/'+userinfo.openid+'logo.jpeg').size(logo_size);
    ...
    .saveAsync(__dirname+'/'+userinfo.openid+'.jpeg',{quality:50},{}, function() {
          return resolve('ok');      
    });
